...
GKE_VERSION=$(gcloud container get-server-config --region us-central1 --flatten="channels" --filter="channels.channel=RAPID" | grep $GKE_K8S_VERSION | head -1 | awk '{print $2}')
